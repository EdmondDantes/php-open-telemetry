...
final class ProtobufSerializer
{
    ...
    {
        return new self(TransportInterface::PROTOBUF);
    }
    ...
    public static function forTransport(TransportInterface $transport): ProtobufSerializer
    {
        return match ($contentType = $transport->contentType()) {
            TransportInterface::PROTOBUF, TransportInterface::JSON, TransportInterface::NDJSON => new self(
                $contentType
            ),
            default => throw new InvalidArgumentException(
                sprintf('Not supported content type "%s"', $contentType)
            ),
        };
    }
    ...
    public function serializeTraceId(string $traceId): string
    {
        return match ($this->contentType) {
            TransportInterface::PROTOBUF                         => $traceId,
            TransportInterface::JSON, TransportInterface::NDJSON => base64_decode(bin2hex($traceId)),
    ...
    {
        return match ($this->contentType) {
            TransportInterface::PROTOBUF           => $spanId,
            TransportInterface::JSON, TransportInterface::NDJSON => base64_decode(bin2hex($spanId)),
            default                  => throw new AssertionError(),
        };
    }
    ...
    {
        return match ($this->contentType) {
            TransportInterface::PROTOBUF => $message->serializeToString(),
            TransportInterface::JSON     => self::postProcessJsonEnumValues($message, $message->serializeToJsonString()),
            TransportInterface::NDJSON   => self::postProcessJsonEnumValues($message, $message->serializeToJsonString()) . "\n",
            default        => throw new AssertionError(),
        };
    }
    ...
    public function hydrate(Message $message, string $payload): void
    {
        switch ($this->contentType) {
            case TransportInterface::PROTOBUF:
                $message->mergeFromString($payload);

                break;
            case TransportInterface::JSON:
            case TransportInterface::NDJSON:
